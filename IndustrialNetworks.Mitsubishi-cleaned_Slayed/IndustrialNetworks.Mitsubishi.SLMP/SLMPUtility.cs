...
public sealed class SLMPUtility
{
	...
	public static KeyValuePair<int, int> GetWordAndBitAddress(Tag tg)
	{
		...
		{
			num = (IsHexadecimal(memory) ? Convert.ToInt32(value, 16) : ((!IsOctal(memory)) ? Convert.ToInt32(value) : Convert.ToInt32(value, 8)));
			num2 = num / 16;
			num %= 16;
		}
		else
		{
			num2 = ((!IsHexadecimal(memory)) ? Convert.ToInt32(value) : Convert.ToInt32(value, 16));
			num = num2 % 16;
		}
		return new KeyValuePair<int, int>(num2, num);
	}
	...
	public static int GetAbsoluteBitAddress(Tag tg)
	{
		string memory = GetMemory(tg);
		string text = tg.Address.Substring(memory.Length, tg.Address.Length - memory.Length);
		int result = 0;
		if (IsBitMemory(memory))
		{
			result = (IsHexadecimal(memory) ? Convert.ToInt32(text, 16) : ((!IsOctal(memory)) ? int.Parse(text) : Convert.ToInt32(text, 8)));
		}
		return result;
	}
	...
	public static string IncrementWordAddress(Device device_0, Tag tg)
	{
		...
		if (!IsBitMemory(memory))
		{
			text = ((!IsHexadecimal(memory)) ? (memory + (Convert.ToInt32(value) + num)) : (memory + (Convert.ToInt32(value, 16) + num).ToString("X")));
		}
		else if (IsHexadecimal(memory))
		{
			text = memory + ((tg.DataType != 0) ? (Convert.ToInt32(value, 16) + num * 16) : (Convert.ToInt32(value, 16) + num)).ToString("X");
		}
		else if (IsOctal(memory))
		{
			int num2 = ((tg.DataType != 0) ? (Convert.ToInt32(value, 8) + num * 16) : (Convert.ToInt32(value, 8) + num));
			text = memory + Convert.ToString(num2, 8);
		}
		else
		{
			text = memory + ((tg.DataType != 0) ? (Convert.ToInt32(value) + num * 16) : (Convert.ToInt32(value) + num));
		}
		tg.Address = text;
		SetTagName(tg);
		return text.ToUpper();
	...
	public static ValidateResult TagValidate(Tag tg)
	{
		...
		if (string.IsNullOrEmpty(validateResult.Message))
		{
			...
			{
				tg.Address = memory + Convert.ToInt32(text, 16).ToString("X");
			}
			else
			{
				tg.Address = memory + int.Parse(text);
			}
			tg.Address.ToUpper();
			if (num)
			{
