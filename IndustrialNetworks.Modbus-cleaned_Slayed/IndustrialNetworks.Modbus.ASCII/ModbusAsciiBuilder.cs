...
public class ModbusAsciiBuilder : ModbusBuilder
{
	...
	public string ReadMessage(byte stationNo, byte func, int address, int numberOfData)
	{
		string text = stationNo.ToString("X2");
		text += func.ToString("X2");
		text += address.ToString("X4");
		text += numberOfData.ToString("X4");
	...
	protected string WriteMultipleMessage(byte stationNo, int address, byte func, int numberOfData, string hex_value)
	{
		string text = stationNo.ToString("X2");
		text += func.ToString("X2");
		text += address.ToString("X4");
