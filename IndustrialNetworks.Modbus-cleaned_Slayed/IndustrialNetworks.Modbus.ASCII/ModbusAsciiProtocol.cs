...
namespace NetStudio.Modbus.ASCII;

public class ModbusAsciiProtocol : ModbusAsciiBuilder, IModbusProtocol
{
	private INetworkAdapter adapter;

	private const int FORMAT_READ = 11;

	private const int FORMAT_WRITE = 13;

	public Author Author => new Author();

	public ModbusAsciiProtocol(INetworkAdapter adapter)
	{
		...
		return await Task.Run(delegate
		{
			...
			{ 
				...
				lock (adapter)
				{
					...
					while ((num != RP.SendMsg.Length || num4 < num3 || (num4 >= num3 && text[0] != ':')) && num2 <= RP.ConnectRetries);
				}
				num4 = text.Length;
				if (num4 >= 11 && text[0] == ':')
				{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				lock (adapter)
				{
					...
					while ((num3 != RP.SendMsg.Length || num5 < num2 || (num5 >= num2 && text[0] != ':')) && num4 <= RP.ConnectRetries);
				}
				num5 = text.Length;
				if (num5 >= 11 && text[0] == ':')
				{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				lock (adapter)
				{
					...
					while ((num2 != text2.Length || text.Length < 13 || (text.Length >= 13 && text[0] != ':')) && num3 <= num);
				}
				if (text.Length >= 13 && (text.Length < 13 || text[0] == ':'))
				{
	...
	public async Task<IPSResult> WriteRegisterAsync(WritePacket WP)
	{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				string text = string.Empty;
				int num3 = 0;
				string text2 = WriteMultipleMessage(WP.StationNo, WP.Address, 16, WP.Quantity, hex_value);
				lock (adapter)
				{
					do
					{
					...
					while ((num != text2.Length || num3 < 13 || (num3 >= 13 && text[0] != ':')) && num2 <= WP.ConnectRetries);
				}
				num3 = text.Length;
				if (num3 >= 11 && text[0] == ':')
				{
	...
	public async Task<IPSResult> WriteCoilAsync(WritePacket WP)
	{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				string text = string.Empty;
				int num3 = 0;
				string text2 = WriteMessage(WP.StationNo, WP.Address, 5, WP.ValueHex);
				lock (adapter)
				{
					do
					{
