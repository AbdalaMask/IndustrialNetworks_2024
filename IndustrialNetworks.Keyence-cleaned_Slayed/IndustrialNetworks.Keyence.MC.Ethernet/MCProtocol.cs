...
public sealed class MCProtocol
{
	...
	public MCProtocol(INetworkAdapter adapter, Frame frame)
	{
		...
		try
		{
			...
			if (num == RP.SendBytes.Length)
			{
				if (array.Length >= 11)
				{
					...
					string text = MCUtility.CheckErrorCodes(BitConverter.ToUInt16(new byte[2]
					{
						array[9],
						array[10]
					}));
		...
		try
		{
			...
			if (num == WP.Data.Length)
			{
				if (array.Length >= 11)
				{
					...
					string text = MCUtility.CheckErrorCodes(BitConverter.ToUInt16(array2));
					if (string.IsNullOrEmpty(text))
					{
						iPSResult.Status = CommStatus.Success;
