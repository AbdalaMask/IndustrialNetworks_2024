...
public class DeltaRtuProtocol : DeltaRtuBuilder, IDeltaMaster
{
	...
	public DeltaRtuProtocol(INetworkAdapter adapter)
	{
	...
	public async Task<IPSResult> WriteCoilAsync(byte stationNo, int address, BOOL[] values)
	{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				int num3 = 0;
				byte[] array = Array.Empty<byte>();
				byte[] array2 = WriteMessage(stationNo, address, 15, values.Length, values2);
				lock (adapter)
				{
					do
					{
	...
	public async Task<IPSResult> WriteCoilAsync(WritePacket WP)
	{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				int num2 = 0;
				byte[] array = Array.Empty<byte>();
				byte[] array2 = WriteMessage(WP.StationNo, WP.Address.BitAddress, 5, WP.DataDec);
				lock (adapter)
				{
					do
					{
	...
	public async Task<IPSResult> WriteRegisterAsync(WritePacket WP)
	{
		...
		return await Task.Run(delegate
		{
			...
			try
			{
				...
				int num2 = 0;
				byte[] array = Array.Empty<byte>();
				byte[] array2 = WriteMessage(WP.StationNo, wordAddress, 16, WP.Quantity, WP.DataDec);
				lock (adapter)
				{
					do
					{
